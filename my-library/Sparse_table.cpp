...
int f(int a, int b) {
	...
	/*
	The operation requires only associativity[1], commutativity[2] and idempotency[3].
	For example: min, max, gcd.
	--------------------------
...
int st[N][K]; // sparse table: st[i][k] = f(arr[i], arr[i + 1], ..., arr[i + 2 ^ k - 1])
...
void build_st() {
	...
	*/
	...
	= f(f(a[i], ..., a[i + 2 ^ (k - 1) - 1]), f(a[i + 2 ^ (k - 1)], ..., a[i + 2 ^ k - 1])) =
...
int get(int l, int r) {
	...
	<IMPORTANT>This request is for the semi-interval [l, r)! Also, l < r.</IMPORTANT>
	*/
	...
	'cause [l, l + 2 ^ k - 1] and [r - 2 ^ k, r - 1] in [l, r) and this segments cover all semi-interval [l, r).
