...
ld &operator/=(ld &a, ld b) { return a = a / b; }
template <class T>
bool operator==(ld a, T b) {
  return my_abs(a.x - b) < EPS;
}
template <class T>
bool operator!=(ld a, T b) {
  return !(a == b);
...
template <class T>
bool operator==(T a, ld b) {
  return my_abs(a - b.x) < EPS;
}
template <class T>
bool operator!=(T a, ld b) {
  return !(a == b);
...
ld double_s(Triangle t) {
  return cross_product(Vector(t.A, t.B), Vector(t.A, t.C));
}
...
bool in(Point P, Angle phi) {
  return P == phi.O ||
         abs(atan2(Vector(phi.O, phi.A), Vector(phi.O, phi.B))) ==
             abs(atan2(Vector(phi.O, phi.A), Vector(phi.O, P))) +
                 abs(atan2(Vector(phi.O, P), Vector(phi.O, phi.B)));
}  // TODO: improve
...
bool cross(Seg q, Seg e) {
  Line n(q), m(e);
  return sign(m(q.A)) * sign(m(q.B)) <= 0 && sign(n(e.A)) * sign(n(e.B)) <= 0;
}  // TODO: improve
bool is_left(Vector a, Vector b) {
  return cross_product(a, b) <= 0;
}  // clockwise
bool is_right(Vector a, Vector b) {
  return cross_product(a, b) >= 0;
}  // counterclockwise
bool is_collinear(Vector a, Vector b) { return cross_product(a, b) == 0; }
bool is_co_directed(Vector a, Vector b) {
  return is_collinear(a, b) && dot_product(a, b) >= 0;
}
bool is_convex_hull(Polygon p) {
  ...
  for (int i = 1; i < (int)p.size(); ++i) {
    if (is_collinear(Vector(p[i - 1], p[i]),
                     Vector(p[i], p[(i + 1) % (int)p.size()])) ||
        flag != is_left(Vector(p[i - 1], p[i]),
                        Vector(p[i], p[(i + 1) % (int)p.size()]))) {
      return false;
    }
  }
...
vector<Point> intersect(Line n, Line m) {
  return is_collinear(n.dir_vec(), m.dir_vec())
             ? n == m ? vector<Point>(2) : vector<Point>(0)
             : vector<Point>({good_intersect(n, m)});
}
...
Circle make_circle(Point A, Point B, Point C) {
  Point O = intersect(Line(mid(B, C), normal(Line(B, C))),
                      Line(mid(A, B), normal(Line(A, B))))[0];
...
Polygon convex_hull(Polygon p) {
  sort(p.begin(), p.end(),
       [](Point A, Point B) { return make_pair(A) < make_pair(B); });
  ...
  for (int i = 0; i < p.size(); ++i) {
    ...
    while (ch.size() >= 2 && is_left(Vector(ch[ch.size() - 2], ch.back()),
                                     Vector(ch.back(), p[i]))) {
      ch.pop_back();
    }
    ch.push_back(p[i]);
...
Polygon minkovski_sum(Polygon P, Polygon Q) {
  ...
  while (i != P.size() - 1 || j != Q.size() - 1) {
    if (j == Q.size() - 1 ||
        (i != P.size() - 1 &&
         cross_product(Vector(P[i], P[i + 1]), Vector(Q[j], Q[j + 1])) < 0)) {
